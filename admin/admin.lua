addCommandHandler('setrank', function (player, command, account, group)
    ...
    -- ensure the account is valid
        ...
        if not accountObj then
            return outputChatBox('The specifed account does not exist.', player, 255, 100, 100)
        end

    -- ensure the group is valid
        local groupObj = aclGetGroup(group)
        if not groupObj then
            return outputChatBox('The specifed group does not exist.', player, 255, 100, 100)
        end
    ...
    -- let the user know that we were successfull
